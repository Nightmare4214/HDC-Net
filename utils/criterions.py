...
def sigmoid_dice_loss(output, target, alpha=1e-5):
    ...
    logging.info('1:{:.4f} | 2:{:.4f} | 4:{:.4f}'.format(1 - loss1.item(), 1 - loss2.item(), 1 - loss3.item()))
...
def GeneralizedDiceLoss(output, target, eps=1e-5, weight_type='square'):  # Generalized dice loss
    ...
    logging.info('1:{:.4f} | 2:{:.4f} | 4:{:.4f}'.format(loss1.item(), loss2.item(), loss3.item()))
...
def expand_target(x, n_class, mode='softmax'):
    """
        ...
        :param x: 4D input image (NxDxHxW)
        :param n_class: number of channels/labels
